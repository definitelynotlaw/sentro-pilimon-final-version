...
const getFromEmail = () => {
  if (fromEmail.includes('@sentropilimon.com')) {
    // Domain not verified yet - use Resend's test sender
    console.warn('sentropilimon.com domain not verified. Using fallback sender.')
    return 'resend@resend.dev'
  }
  return fromEmail
}

// GET - Test if Resend is configured correctly
export async function GET() {
  if (!resendApiKey || resendApiKey === 'placeholder_resend_key') {
    return NextResponse.json({
      success: false,
      error: 'RESEND_API_KEY not configured or is placeholder',
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({
        ...
        hint: 'Check Resend dashboard for API key validity'
      })
    }
