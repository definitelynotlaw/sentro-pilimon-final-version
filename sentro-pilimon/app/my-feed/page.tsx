...
async function getFollowedChannelsFeed(userId: string) {
  ...
  // Query announcements from followed channels
  let query = supabase
    .from('announcements')
    .select(`
...
export default async function MyFeedPage() {
  ...
  const announcements = await getFollowedChannelsFeed(user.id)
    ...
    <main className="min-h-screen pb-20 md:pb-0">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <p className="mb-6" style={{ color: '#5A5A56' }}>
          Announcements from channels you follow
        </p>
